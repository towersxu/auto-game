...
async function hasLabel(owner, repo, issueNumber, labelName) {
  try {
    const issue = await githubRequest(`/repos/${owner}/${repo}/issues/${issueNumber}`);
    return issue.labels && issue.labels.some(label => label.name === labelName);
  } catch (error) {
...
async function addLabel(owner, repo, issueNumber, labelName) {
  try {
    await githubRequest(`/repos/${owner}/${repo}/issues/${issueNumber}/labels`, 'POST', {
      labels: [labelName]
    });
...
async function removeLabel(owner, repo, issueNumber, labelName) {
  try {
    await githubRequest(`/repos/${owner}/${repo}/issues/${issueNumber}/labels/${labelName}`, 'DELETE');
...
async function createDetailedComment(owner, repo, issueNumber, steps, summary) {
  let commentBody = `## ✅ Task Completed\n\n`;
  commentBody += `### 处理步骤\n\n`;
  for (let i = 0; i < steps.length; i++) {
    commentBody += `${i + 1}. ${steps[i]}\n`;
  }
  commentBody += `\n### 改动思路\n\n${summary}\n\n`;
  ...
  console.log(`Detailed comment added to issue #${issueNumber}`);
}

async function createFailureComment(owner, repo, issueNumber, errorMessage, steps = []) {
  let commentBody = `## ❌ Task Failed\n\n`;
  if (steps.length > 0) {
    commentBody += `### 已完成的步骤\n\n`;
    for (let i = 0; i < steps.length; i++) {
      commentBody += `${i + 1}. ${steps[i]}\n`;
  ...
  console.log(`Failure comment added to issue #${issueNumber}`);
}
...
Commands:
  ...
  start              Start working on an issue
                     ...
                     Adds 'ai-doing' label to prevent duplicate processing

  submit             Submit completed work for an issue
                     Args: <issue-number>
                     Options: --message "commit message"
                              --steps "step1;step2;step3" (semicolon-separated steps)
                              --summary "summary text"
                     Commits changes, pushes branch, creates PR, and comments on issue
                     Removes 'ai-doing' label on completion

  fail               Report task failure
                     Args: <issue-number>
                     Options: --error "error message"
                              --steps "step1;step2" (completed steps before failure)
                     Adds failure comment and removes 'ai-doing' label
...
Examples:
  ...
  node index.js submit 1 --message "Implement feature X" --steps "Read code;Modify function;Test" --summary "Added new feature"
  node index.js fail 1 --error "Build failed" --steps "Read code;Started modification"
`);
}
...
async function startTask(issueNumber) {
  ...
  const hasAiDoingLabel = await hasLabel(owner, repo, issueNumber, 'ai-doing');
  if (hasAiDoingLabel) {
    console.error(`Issue #${issueNumber} already has 'ai-doing' label. Another agent may be working on it.`);
    console.error('If you are sure no one is working on it, remove the label manually and try again.');
    process.exit(1);
  }

  await addLabel(owner, repo, issueNumber, 'ai-doing');
  ...
  console.log(`\n✅ Ready to work on issue #${issueNumber}`);
...
async function submitTask(issueNumber, commitMessage, steps = [], summary = '') {
  const repoPath = process.env.GITHUB_REPOSITORY || getRepoInfo();
  const [owner, repo] = repoPath.split('/');
  ...
  try {
    ...
    if (steps.length > 0 || summary) {
      await createDetailedComment(owner, repo, issueNumber, steps, summary || '任务已完成');
    }

    await removeLabel(owner, repo, issueNumber, 'ai-doing');
    ...
    await createFailureComment(owner, repo, issueNumber, error.message, steps);
    await removeLabel(owner, repo, issueNumber, 'ai-doing');
    throw error;
  }
}

async function reportFailure(issueNumber, errorMessage, steps = []) {
  const repoPath = process.env.GITHUB_REPOSITORY || getRepoInfo();
  const [owner, repo] = repoPath.split('/');

  console.log(`Reporting failure for issue #${issueNumber}...`);

  await createFailureComment(owner, repo, issueNumber, errorMessage, steps);
  await removeLabel(owner, repo, issueNumber, 'ai-doing');
...
function parseArgs() {
  ...
  if (options.steps && typeof options.steps === 'string') {
    options.steps = options.steps.split(';').map(s => s.trim()).filter(s => s);
  }
  ...
  switch (command) {
    ...
    case 'submit':
      ...
      submitTask(issueNumber, options.message, options.steps || [], options.summary || '');
      break;
    case 'fail':
      if (!issueNumber) {
        console.error('Error: Issue number is required for fail command');
        process.exit(1);
      }
      reportFailure(issueNumber, options.error || 'Unknown error', options.steps || []);
      break;
    default:
