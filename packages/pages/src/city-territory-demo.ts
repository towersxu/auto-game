...
const worldMap = new WorldMap(GRID_WIDTH, GRID_HEIGHT);
...
map.onCellClick((col, row) => {
  ...
  if (!worldMap.isAvailable(coord)) {
    ...
    statusEl.textContent = `(${coord.x}, ${coord.y}) 已被 ${occupant?.name ?? '未知城市'} 占领`;
    return;
  }
  ...
  if (city1.isAdjacentTo(coord)) {
    ...
    statusEl.textContent = `✓ (${coord.x}, ${coord.y}) 并入 City 1！`;
    return;
  }
  ...
  if (city2.isAdjacentTo(coord)) {
    ...
    statusEl.textContent = `✓ (${coord.x}, ${coord.y}) 并入 City 2！`;
    return;
  }

  statusEl.textContent = `(${coord.x}, ${coord.y}) 与任何城市不相邻`;
});
